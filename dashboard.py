...
def init_connection():
    ...
    return psycopg2.connect(
        user=os.getenv("DB_USER"), password=os.getenv("DB_PASS"), host=os.getenv("DB_HOST"), port=os.getenv("DB_PORT"), database=os.getenv("DB_NAME")
    )
...
plot_col, pick_col = st.columns([3, 1], gap="large")
all_time_trend = df_base.groupby(["service_date", "lineref"])["delaysecs"].mean().reset_index()

with pick_col:
    selected = st.multiselect(
        "Lines",
        options=all_time_trend["lineref"].unique(),
        default=all_time_trend['lineref'].unique()[:5]
    )
    new_all = all_time_trend[all_time_trend["lineref"].isin(selected)]

with plot_col:
    fig4 = px.line(new_all, x="service_date", y="delaysecs", color="lineref",
